 ...
 */
const tagsData = (obj) => obj.tags.flat(2);
// console.log(tagsData(accounts[0]));
 ...
 */
const checkTag = (obj) => obj.tags.flat().includes("consectetur");
// console.log(checkTag(accounts[0]));
 ...
 */
const allTags = (arr) => arr.map(({ tags }) => tags.flat()).join(",");
 ...
 */
const allfriends = (arr) =>
  arr.map(({ friends }) => friends.map(({ name }) => name)).flat();
// console.log(allfriends(accounts));
/**
 * Exercise 05
 *
 * Get a list of all the friends for all users who are younger than 30 AND have a balance more than $1500
 */
const allfriendsWithCondition = (arr, ageCond, bal) =>
  arr
    .filter(
      ({ balance, age }) =>
        age <= ageCond && parseInt(balance.replace(/\D/g, "")) > bal
    )
    .map(({ friends }) => friends)
    .flat()
    .map(({ name }) => name);
